...
let idx=0;let answers=[];let latestResultText="";let latestScores=null;let latestTypes=null;let partnerData=null;let latestCoupleText="";
...
function parsePartnerFromUrl(){
  ...
  if(!data)return null;
  try{return decodePayload(data)}
  catch(e){
    console.warn("partner parse failed",e);
    const notice=document.getElementById("inviteNotice");
    if(notice){
      notice.classList.remove("hidden");
      notice.innerText="配对邀请链接读取失败，可能是链接被截断或改写。请让对方重新点击“邀请朋友配对分析”复制完整链接。";
    }
    return null;
  }
}
...
document.querySelectorAll(".options button").forEach(btn=>{btn.onclick=()=>{answers[idx]=Number(btn.dataset.score);idx++;if(idx>=QUESTIONS.length)showResult();else render()}});
...
function calcScores(){
  const raw={};dims.forEach(([k])=>raw[k]=0);
  QUESTIONS.forEach((q,i)=>raw[q.dimension]+=answers[i]||0);
  const scores={};dims.forEach(([k])=>scores[k]=Math.round(raw[k]/24*100));
  return scores;
}
...
function showResult(){
  ...
  latestScores=calcScores();latestTypes=archetypes(latestScores);
  const main=latestTypes[0],sub=latestTypes[1];
  if(partnerData){showCoupleResult(partnerData,{scores:latestScores,types:latestTypes});return}
  ...
  drawRadar("radar",latestScores);renderScores(latestScores);renderAnalysis(latestScores);
...
function makeInvitePayload(){
  return encodePayload({scores:latestScores,types:latestTypes.slice(0,2).map(t=>({name:t.name,line:t.line,desc:t.desc})),createdAt:Date.now()});
}
...
async function copyText(text,msg="已复制"){try{await navigator.clipboard.writeText(text);alert(msg)}catch(e){alert(text)}}

function renderScores(s){
  const box=document.getElementById("scores");box.innerHTML="";
  dims.forEach(([k,n])=>{const div=document.createElement("div");div.className="item";div.innerHTML=`<strong>${n}：${s[k]}</strong><div class="scorebar"><div style="width:${s[k]}%"></div></div>`;box.appendChild(div)});
}
function addItem(id,text,cls=""){const div=document.createElement("div");div.className="item "+cls;div.innerText=text;document.getElementById(id).appendChild(div)}
...
function compatibility(a,b){
  const s1=a.scores,s2=b.scores,parts=[];
  function sim(k,weight,label){const diff=Math.abs(s1[k]-s2[k]);const score=Math.max(0,100-diff);parts.push({k,label,score,weight,diff,a:s1[k],b:s2[k]});return score*weight}
  let total=0,weight=0;
  [["commitment",1.25,"长期承诺"],["reality",1.2,"现实规划"],["communication",1.15,"沟通修复"],["emotion",1.1,"情绪稳定"],["boundary",1.0,"边界节奏"],["intimacy",1.0,"亲密需求"],["security",0.9,"安全感"],["empathy",0.9,"共情照顾"]].forEach(([k,w,label])=>{total+=sim(k,w,label);weight+=w});
  return {score:Math.round(total/weight),parts:parts.sort((x,y)=>y.score-x.score)};
}
function showCoupleResult(a,b){
  ...
  drawCoupleRadar("coupleRadar",a.scores,b.scores);renderCoupleScores(c);renderCoupleAnalysis(a,b,c);
...
function renderCoupleScores(c){
  const box=document.getElementById("coupleScores");box.innerHTML="";
  c.parts.forEach(p=>{const div=document.createElement("div");div.className="item";div.innerHTML=`<strong>${p.label}：${p.score}</strong><div class="mini">对方 ${p.a}｜我 ${p.b}｜差异值 ${p.diff}</div><div class="scorebar"><div style="width:${p.score}%"></div></div>`;box.appendChild(div)});
}
function topStrengths(s){return dims.map(([k,n])=>({k,n,v:s[k]})).sort((a,b)=>b.v-a.v).slice(0,3)}
function lowPoints(s){return dims.map(([k,n])=>({k,n,v:s[k]})).sort((a,b)=>a.v-b.v).slice(0,2)}
function renderCoupleAnalysis(a,b,c){
  ...
  addItem("coupleStrengths",`对方优势维度：${topStrengths(a.scores).map(x=>`${x.n}${x.v}`).join("、")}。`);
  addItem("coupleStrengths",`你的优势维度：${topStrengths(b.scores).map(x=>`${x.n}${x.v}`).join("、")}。`);
  const close=c.parts.filter(p=>p.score>=80).slice(0,3);
  if(close.length)close.forEach(p=>addItem("coupleStrengths",`共同优势：你们在「${p.label}」上比较接近，关系中更容易形成共同节奏。`,"good"));
  const far=c.parts.filter(p=>p.score<65).slice(0,3);
  if(far.length)far.forEach(p=>addItem("coupleRisks",`差异点：「${p.label}」差异明显。对方 ${p.a}，你 ${p.b}，容易成为相处摩擦点。`,"warn"));
  else addItem("coupleRisks","目前没有特别突出的结构性差异，后续重点观察真实相处中的一致性。","good");
  addItem("coupleRisks",`对方相对短板：${lowPoints(a.scores).map(x=>`${x.n}${x.v}`).join("、")}。`);
  addItem("coupleRisks",`你的相对短板：${lowPoints(b.scores).map(x=>`${x.n}${x.v}`).join("、")}。`);
...
function drawRadar(canvasId,s){
  const c=document.getElementById(canvasId),ctx=c.getContext("2d"),w=c.width,h=c.height,cx=w/2,cy=h/2,R=145;
  ctx.clearRect(0,0,w,h);const labels=dims.map(d=>d[1]),vals=dims.map(d=>s[d[0]]),n=labels.length;
  ctx.strokeStyle="#ddd";ctx.fillStyle="#666";ctx.font="13px sans-serif";
  for(let ring=1;ring<=4;ring++){ctx.beginPath();for(let i=0;i<n;i++){const a=-Math.PI/2+i*2*Math.PI/n,r=R*ring/4,x=cx+Math.cos(a)*r,y=cy+Math.sin(a)*r;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.closePath();ctx.stroke()}
  for(let i=0;i<n;i++){const a=-Math.PI/2+i*2*Math.PI/n;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.cos(a)*R,cy+Math.sin(a)*R);ctx.stroke();const lx=cx+Math.cos(a)*(R+34),ly=cy+Math.sin(a)*(R+34);ctx.fillText(labels[i],lx-28,ly+4)}
  ctx.beginPath();vals.forEach((v,i)=>{const a=-Math.PI/2+i*2*Math.PI/n,r=R*v/100,x=cx+Math.cos(a)*r,y=cy+Math.sin(a)*r;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.closePath();ctx.fillStyle="rgba(0,0,0,.14)";ctx.fill();ctx.strokeStyle="#111";ctx.lineWidth=2;ctx.stroke();
}
function drawCoupleRadar(canvasId,s1,s2){drawRadarBase(canvasId);drawRadarShape(canvasId,s1,"rgba(0,0,0,.16)","#111");drawRadarShape(canvasId,s2,"rgba(177,0,62,.16)","#b1003e")}
function drawRadarBase(canvasId){
  const c=document.getElementById(canvasId),ctx=c.getContext("2d"),w=c.width,h=c.height,cx=w/2,cy=h/2,R=145,n=dims.length;
  ctx.clearRect(0,0,w,h);ctx.strokeStyle="#ddd";ctx.fillStyle="#666";ctx.font="13px sans-serif";
  for(let ring=1;ring<=4;ring++){ctx.beginPath();for(let i=0;i<n;i++){const a=-Math.PI/2+i*2*Math.PI/n,r=R*ring/4,x=cx+Math.cos(a)*r,y=cy+Math.sin(a)*r;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.closePath();ctx.stroke()}
  for(let i=0;i<n;i++){const a=-Math.PI/2+i*2*Math.PI/n;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.cos(a)*R,cy+Math.sin(a)*R);ctx.stroke();const lx=cx+Math.cos(a)*(R+34),ly=cy+Math.sin(a)*(R+34);ctx.fillText(dims[i][1],lx-28,ly+4)}
  ctx.fillStyle="#111";ctx.fillText("对方",20,25);ctx.fillStyle="#b1003e";ctx.fillText("我",20,45);
}
function drawRadarShape(canvasId,s,fill,stroke){
  const c=document.getElementById(canvasId),ctx=c.getContext("2d"),w=c.width,h=c.height,cx=w/2,cy=h/2,R=145,n=dims.length,vals=dims.map(d=>s[d[0]]);
  ctx.beginPath();vals.forEach((v,i)=>{const a=-Math.PI/2+i*2*Math.PI/n,r=R*v/100,x=cx+Math.cos(a)*r,y=cy+Math.sin(a)*r;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.closePath();ctx.fillStyle=fill;ctx.fill();ctx.strokeStyle=stroke;ctx.lineWidth=2;ctx.stroke();
}
async function saveCardImage(cardId,filename,title,text){
  const btn=event?.target,old=btn?.innerText;
  try{
    if(btn)btn.innerText="正在生成图片...";
    const canvas=await html2canvas(document.getElementById(cardId),{scale:2,backgroundColor:"#ffffff",useCORS:true});
    const image=canvas.toDataURL("image/png");
    if(navigator.share&&navigator.canShare){
      const blob=await(await fetch(image)).blob();const file=new File([blob],filename,{type:"image/png"});
      if(navigator.canShare({files:[file]})){await navigator.share({title,text,files:[file]});if(btn)btn.innerText=old;return}
    }
    const link=document.createElement("a");link.href=image;link.download=filename;document.body.appendChild(link);link.click();document.body.removeChild(link);if(btn)btn.innerText=old;
  }catch(err){console.error(err);alert("生成图片失败，请稍后再试");if(btn)btn.innerText=old}
}
